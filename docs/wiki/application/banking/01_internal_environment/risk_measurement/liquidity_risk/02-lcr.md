...
- **Low risk** — high credit standing of the issuer, low sensitivity to interest rate and [[wiki/application/banking/01_internal_environment/risk_measurement/market_risk/02-models|market risk]], low legal and inflation risk, denomination in a convertible currency with low FX risk.
...
A competitor analysis is essential — banks typically aim to offer deposit rates in the middle of the competitive range to avoid both under-pricing (losing deposits) and over-paying ([[04-nii_nim|NIM]] compression). In South Africa, competitor information can be sourced from the monthly **BA900 Economic Returns** published by the [[sarb|SARB]] and from competitors' quarterly/annual [[bis|Basel]] Pillar 3 reports.

> **<!-- GAP -->** *Liquidity stress testing scenarios beyond the LCR (pure idiosyncratic, pure market-wide, combined; reverse stress testing for liquidity; ILAAP linkage) are referenced indirectly via the LCR scenario but not separately treated. F107 priority area. See lecture `02-lcr.html` section 9 for the interim treatment.*
