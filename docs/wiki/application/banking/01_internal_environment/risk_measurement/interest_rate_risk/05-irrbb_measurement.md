...
1. [[04-nii_nim|NII]] is squeezed as the cost of liabilities increases while asset yields stay fixed for an extended period.
...
**Key trade-off:** stabilising [[04-nii_nim|NII]] (earnings) vs stabilising EVE involves opposing structural positions. A bank needs to consider both perspectives when managing IRRBB. [[basel_framework|Basel III]] requires banks to measure the impact of interest rate shocks on EVE (six prescribed shock scenarios) and [[04-nii_nim|NII]] (two shock scenarios) as part of [[01-pillar_2b|Pillar 2]] supervisory review.

## Earnings at Risk (EaR)

The earnings perspective focuses on how rate changes affect the bank's [[04-nii_nim|NII]] and earnings over a given time horizon (typically short to medium term, not more than 2 years). Accuracy of the forecast decreases as the time horizon increases.
...
The bank has R10m more liabilities repricing over the next 12 months than assets. An interest rate increase will therefore reduce [[04-nii_nim|NII]] over this period, as more liabilities reprice to higher rates than assets.

### Step 2: [[04-nii_nim|NII]] Sensitivity

Given a change in rates, the [[04-nii_nim|NII]] sensitivity in each time band is:
...
A 100bp increase in rates reduces [[04-nii_nim|NII]] by R225,000 over the following year.
...
The final step introduces a stochastic element. An appropriately calibrated stochastic interest rate process (e.g. the Vasicek model) is used to generate a distribution of potential interest rate scenarios at a given confidence level. Monte Carlo simulations then produce a distribution of [[04-nii_nim|NII]] changes over the time horizon.

```math
\text{EaR}_{1-\alpha} = \text{VaR}_{1-\alpha}(\Delta\text{NII})
```

EaR is effectively the VaR with respect to the bank's [[04-nii_nim|NII]] for a given confidence level over the time horizon of interest. For example, the 95% EaR is the level such that only 5% of simulations produce a worse [[04-nii_nim|NII]] outcome.
