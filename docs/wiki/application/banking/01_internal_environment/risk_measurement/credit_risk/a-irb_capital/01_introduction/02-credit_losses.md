---
tags:
  - application/banking/internal-environment/risk-measurement/credit-risk/airb-capital/introduction/credit-losses
  - difficulty/unknown
...
**Non-defaulted:** 
$$
\text{EL} = \text{PD}_\text{TTC} × \text{LGD}_\text{DT} × \text{EAD}_\text{Reg}
$$
**Defaulted:** 
$$
\text{EL} = 100\% × \text{ELBE} × \text{EAD}_\text{Actual}
$$
...
### Systemically Conditional PiT PD
...
$$
\text{ELBE} = P(\text{cure}) × \text{LGD}_\text{cure} + P(\text{no cure}) × \text{LGD}_\text{if no cure}
$$

Where:
- $P(\text{cure})$ = probability the borrower recovers and resumes payments
- $\text{LGD}_\text{cure}$ ≈ 0 (minimal loss if borrower rehabilitates)
- $\text{LGD}_\text{if no cure}$ = expected loss rate through workout/enforcement

> Why not use a LRA LGD instead of the ELBE/BEEL?
...
> Why not use a downturn LGD instead of the ELBE/BEEL?
...
> Why not use a ECL LGD instead of the ELBE/BEEL?
