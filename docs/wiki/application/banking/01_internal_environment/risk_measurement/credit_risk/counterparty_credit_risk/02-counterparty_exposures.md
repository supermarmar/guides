...
- Can be implemented with existing [[wiki/application/banking/01_internal_environment/risk_measurement/market_risk/02-models|market risk]] infrastructure
...
[[bis|Basel]] specifically makes allowance for the Monte Carlo approach in its framework for modelling [[wiki/application/banking/01_internal_environment/risk_measurement/market_risk/02-models|market risk]] under the internal model approach (akin to the IRBA for credit risk). The result of the Monte Carlo simulation feeds directly into the EEPE calculation:
...
Wrong-way risk is partially addressed by the 1.4 alpha factor in both the SA-CCR and IMM formulas, but sophisticated banks additionally model it explicitly within their Monte Carlo simulations by correlating [[wiki/application/banking/01_internal_environment/risk_measurement/market_risk/02-models|market risk]] factors with counterparty default events.
