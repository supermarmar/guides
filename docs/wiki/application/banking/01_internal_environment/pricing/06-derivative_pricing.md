...
This allows a bank to safely fund a fixed-rate loan (earning 4%) with variable-rate deposits (paying 3m LIBOR). Without the swap, rising LIBOR compresses [[04-nii_nim|NIM]]; with the swap, the bank locks in a fixed net margin regardless of LIBOR:
