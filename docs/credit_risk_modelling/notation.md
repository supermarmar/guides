...
|Cure indicator| $C_{i,t}(p)= \left\{\begin{array}{ll} 1 & d_{i,t}=0 \text{ and } J_{i,t}(p)=0 \\ 0 & \text{otherwise} \\ \end{array} \right.$ for $t\geq p$ | the cure status of a loan defined by whether it is not in probation and not in instant default |
|Default indicator (probabtion period)| $D_{i,t}(p)=[C_{i,t}(p) = 0]$ | if a loan has not cured, it is still considered to be "in" default |
|Write-off indicator| $W_{i,t}(w)=G_{i,t}(3,w)$ where $w\geq 0$ is a specified write-off threshold/point| if a loan has 3 or more missed payments for $w$ months it is considered written-off |
|Delinquency outcome| $G^*_{i,t}(k,d,s)=G_{i,t+k}(d,s)$ | a loan's future delinquency status for a specified outcome period |
|Default outcome| $D^*_{i,t}(k,p)=D_{i,t+k}(p)$ | a loan's future default outcome for a specified outcome period |
|Behavourial loan dataset|$I=\{i,t,t',x_{i},x_{i,t},x_{t'},g_{i,t},G_{i,t}(d,s),G^*_{i,t}(k,d,s),H_{i,t}(p),C_{i,t}(p),D_{i,t}(p),D^*_{i,t}(p,k),W_{i,t}(w)\}$|set of the behaviour of all over the entire observable period|
...
|$k$-month forward default rate (number weighted)| $r(k,x_{i},x_{i,t},t')=\frac{1}{n_0(x_{i},x_{i,t},t')}\displaystyle \sum_{i\subset I(x_{i},x_{i,t},t')}[D^*_{i,t}(p,k) = 1]\times [D_{i,t}(p) = 0]$| worst-ever $k$-month default rate in a calendar time (point-in-time) given a population of non-defaulted loans for a cohort group|
|$k$-month forward default rate (exposure weighted)| $r_\text{Bal}(k,x_{i},x_{i,t},t')=\frac{1}{b_0(x_{i},x_{i,t},t')}\displaystyle \sum_{i\subset I(x_{i},x_{i,t},t')}[D^*_{i,t}(p,k) = 1]\times [D_{i,t}(p) = 0]\times \text{Balance}_{i,t}$| worst-ever $k$-month default rate in a calendar time (point-in-time) given a population of non-defaulted loans for a cohort group, weighted by balance / exposure|
|$k$-month average default rate | $\bar r(k,x_{i},[t_a',t_b'])=\frac{1}{b-a}\displaystyle \sum_{t'\subset [t_a',t_b'] } r(k,x_{i},x_{i,t},t')$| average of $k$-month forward default rates over a specified time period|
...
|$k$-month unconditional PiT marginal PD| $\text{PD}_{i,t}^\text{uPiT}(k,x_{i},x_{i,t})=P(D^*_{i,t}(p,k) = 1\|X_i=x_i,X_{i,t}=x_{i,t})$ |  $k$-month unconditional probability of default for a loan from a particular cohort group, regardless of prior default or prepayment|
|$k$-month conditional PiT marginal PD| $\text{PD}_{i,t}^\text{PiT}(k,x_{i},x_{i,t})=P(D^*_{i,t}(p,k) = 1 \| D_{i,t}(p)=0,X_i=x_i,X_{i,t}=x_{i,t})$ | worst-ever $k$-month conditional probability of default for a non-defaulted loan from a particular cohort group|
|$k$-month survival probability| $\text{S}_{i,t}(k,x_{i},x_{i,t})=\text{S}_{i,t-1}(k,x_{i},x_{i,t-1})\times (1- \text{PD}_{i,t}^\text{PiT}(k,x_{i},x_{i,t}))$ | $k$-month conditional probability of a non-defaulted loan remaining in a performing state from a particular cohort group|
|$k$-month cumulative PD| $\text{PD}_{i,t}^\text{Cum}(k,x_{i})=1-\displaystyle \prod_{v=0}^t \text{S}_{i,v}(k,x_{i},x_{i,v})=\displaystyle \sum_{v=1}^t \text{PD}_{i,v}^\text{PiT}(k,x_{i},x_{i,v})$ | $k$-month probability of default at any point up before a specified loan age, for a non-defaulted loan from a particular cohort group|
|$k$-month lifetime PD| $\text{PD}_{i}^\text{Life}(k,x_{i})=\text{PD}^\text{Cum}_{i,\infty}(k,x_{i})$ | $k$-month probability of default throughout the entire lifetime of a loan, for a non-defaulted loan from a particular cohort group|
|$k$-month PD term structure|$\text{PD}_{i}^\text{Term}(k,x_{i})=\{ \text{PD}^\text{PiT}_t(k,x_{i},x_{i,t}) \| \forall t \subset [1,\infty] \}$|series of conditional marginal PiT PDs over discrete time intervals (e.g., monthly or annually) for the life of the exposure, typically manifest as a non-linear and right-skewed curve over the loan term|
...
|$k$-month conditional FiT PD| $\text{PD}_{i,t,t'}^\text{FiT}(k,x_{i},x_{i,t})=P(D^*_{i,t}(p,k) = 1 \| D_{i,t}(p)=0,X_i=x_i,X_{i,t}=x_{i,t})\times\text{FLI}_{t'}$ | future or forward macro-economic adjusted $k$-month conditional probability of a non-defaulted loan from a particular cohort group |
|$k$-month systemically conditional PiT marginal PD| $\text{PD}_{i,t}^\text{SysPiT}(k,x_{i},x_{i,t}\|\text{FLI}_{t'}) = N(\large\frac{N^{-1}(\text{PD}_{i}^\text{TTC}(k,x_{i},[t_a',t_b']))+\text{FLI}_{t'}\sqrt{\rho}}{\sqrt{1-\rho}})$ | $k$-month probability of default for a non-defaulted loan conditional not necessarily on their individual non-defaulted status but instead the macro-economic environment |
|$k$-month EAD|$\text{EAD}_{i,t}(k)= [D^*_{i,t}(p,k)=1]\times \text{Balance}_{i,t+k}$| balance of a loan in $k$ months that is in default at that observation date |
