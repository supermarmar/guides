...
between two SICR-rates. The EBA-threshold (𝑢 = 200%) achieves a respectable MAE-value, which is reasonably close to that of the SICR-model, albeit still worse. More importantly, the MAE-value of the EBA-threshold is more than 7 times lower than that of the best-AUC threshold (𝑢 = 100%). Despite its improved prediction accuracy at the account-level, the best-AUC threshold clearly results in an overly conservative SICR-rate at the portfolio-level, which surely poses an immense opportunity cost.
