...
The [[01-target_variable|target variable]] can then be estimated at the portfolio-level by the 𝑣-month default rate, defined over 𝑡' for a given $D'$ as:
...
The method above calculates default rates by considering the number of accounts, treating all defaults equally, regardless of exposure size. This is used when the focus is on account-level performance, customer [[06-segmentation|segmentation]], or building behavioral scorecards. Useful for corporate business banking when there is low number of accounts.
