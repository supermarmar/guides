...
Often capital is spoken of as being “held” or “put aside” by a bank in order to support lending operations, as if it were an asset. Far from being an asset, capital is always shown on the liability side of the balance sheet alongside all the other forms of liabilities the bank has, and as such a
form of funding for the bank.
...
International guidelines for adequate capital holdings are set out by the Basel framework. These guidelines set out what instruments can be considered as capital as well as how to calculate capital requirements. As these are only guidelines, national regulators can decide how to implement the guidelines and what additions might need to be made, determined by local conditions.
...
Regulatory requirements are those of the national regulator and international regulatory considerations as set by the Basel framework. In the United Kingdom, for example, the requirements would include the Capital Requirements Directives IV (the European Union’s legislative implementation of Basel III), the buffer requirements of the UK Prudential Regulatory Authority (PRA), the leverage ratio, and any relevant ring-fencing rules.

For a bank in South Africa, the considerations are similar as the SARB is a member of the Basel Committee, with the Prudential Authority wing of the SARB being responsible for the implementation of the banking prudential regulation. The SARB compares the capital strategies of banks by type of bank, with smaller banks having considerably higher capital requirements than their larger counterparts given the lack of diversification in their portfolio and potentially due to higher business risk where a bank, for example, focuses on unsecured lending.

The capital a bank is required to hold is highly regulated. Indeed, Basel III also sets a limit on how much balance sheet leverage a bank can employ. Both Pillar 1 and Pillar 2 of the Basel framework set out calculations or guidelines for capital requirements. The capital held under the Basel framework will constitute the regulatory capital of the bank.

The total required capital of the bank will be the Pillar 1 requirement as well as any additions from the Pillar 2 requirements.

The table below shows the capital requirements of a bank as per the Basel III guidelines.
...
Pillar 1 uses the risk-weighted assets (RWA) method. Each type of asset the bank holds is assigned a weight and the combined total RWAs multiplied by the Basel minimum required capital percentage, leads to the minimum required Pillar 1 capital holding.
...
The last type of capital buffer is the systemically important banks’ buffer. This requirement only applies to large banks which pose a systemic risk as determined by the regulator. The regulator can require a bank to hold up to an additional 3% of RWAs in CET1 if it is determined to be a systemically important bank.
