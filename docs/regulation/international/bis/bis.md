...
Counterparty credit risk (CCR) is the risk that a counterparty to a trade defaults before settlement — a bilateral risk applicable to OTC derivatives, exchange-traded derivatives, long settlement transactions, and securities financing transactions (repo). The CVA capital charge complements CCR by capturing mark-to-market losses from counterparty credit *deterioration* (not just default), which were twice as large as CCR default losses during the 2007–09 financial crisis.
...
Under the 2014 Basel III reforms, the old Current Exposure Method (CEM) and Standardised Method (SM) for measuring CCR EADs were replaced by the **Standardised Approach for CCR (SA-CCR)** and the **Internal Model Method (IMM)**. The CVA capital charge was introduced as part of Basel III (2010) and was substantially revised in the 2017 Basel III finalisation (effective 2019), replacing the internally modelled CVA approach with a standardised approach (SA-CVA) and a basic approach (BA-CVA). For the full computation mechanics of SA-CCR, IMM, and CVA capital charges, see [Regulatory Capital (Trading Book)](..\..\..\application\banks\credit_risk\05_counterparty_modelling\01_introduction\01-regulatory_capital.md). For the mathematical foundations of EE/EPE/EEPE and survival curve construction, see [Counterparty Exposures](..\..\..\application\banks\credit_risk\05_counterparty_modelling\01_introduction\02-counterparty_exposures.md).
